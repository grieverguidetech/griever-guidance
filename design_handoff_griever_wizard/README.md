...
This handoff covers two complete wizard flows, twelve phone screens total:
...
Both flows start from the same template picker.
...
- Title `Griever Guidance`; sub-line "Choose the kind of message you'd like to send."
...
1. **Their name** — "e.g. Margaret Hayes"
2. **Service date** — "e.g. June 14, 2026"
3. **Time** — "e.g. 2:00 PM"
4. **Location** — **Google Maps Places typeahead** (see below)
5. **Notes** *(optional)* — textarea, 2 rows, "Reception to follow at the house"
...
Design principle: this message goes out within hours. It must be sendable on **name + date
alone**, and must never block on details the user does not have yet.

### B1 — Only what's needed
Title "Announce the passing", sub-line "Two answers are enough. You can add the rest later."

1. **Their name** — required
2. **Date they passed** — required
3. **Your name** *(optional)* — "So people know who wrote"
4. **Anything you'd like to add** *(optional)* — textarea, 3 rows
5. **Link to the obituary** *(optional)* — "Paste a link, if there is one", with a 12px hint
   below: "If you add one, we'll include it at the end of the message."

Primary `Continue`, then a centered italic 12px line: "Everything is saved as you go. You can
stop and come back." — this is a promise the implementation must keep: **persist form state
locally on every change** and restore it on relaunch.

*Obituary link*: validate loosely (accept bare domains, prepend `https://`), never hard-fail.
When present, append it as the final line of the SMS.
...
- `deceasedName`, `dateOfPassing`, `senderName`, `personalNote`, `obituaryUrl`
...
| A2-details-form.png | A2 — Details form (Places typeahead open) |
...
| B1-announcement-form.png | B1 — Only what's needed |
...
| B1 — Announcement form | *new* (nearest: `DetailsForm.tsx`) |
