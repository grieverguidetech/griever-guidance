 ...
 * lives outside any lib/app's `src/` (in `apps/gateway`'s sibling `convex/`
 * folder used by `npx convex dev`), and `apps/gateway`'s own non-bundled
 * esbuild config only compiles/copies its own `src/**` — a relative import
 * reaching outside it resolves fine against the source tree (so `typecheck`
 * never catches it) but breaks at runtime once esbuild's output path
 * restructures around that outside reference. Referencing functions by name
